...
- **임의 웹페이지** — 자기완결 플레이리스트를 내보내는 페이지는 WebKit 웹뷰로
  로드해 **페이지가 스스로 요청하는 미디어 URL을 가로채** 재생합니다(사이트별
  코드 없음). `yt-dlp`가 지원하는 페이지는 그쪽도 시도합니다.
...
모든 기능이 커맨드로 노출됩니다(`sok plugin.soksak-playbox.<name>` / MCP):
